...
bool interectRayAABB(glm::vec3 p1, glm::vec3 p2, glm::vec3 d, AbstractModel::AABB aabb) {
	float tmin = -FLT_MAX, tmax = FLT_MAX;
	if (!testSLABPlane(p1.x, d.x, aabb.mins.x, aabb.mins.x, tmin, tmax))
		return false;
	if (!testSLABPlane(p1.y, d.y, aabb.mins.y, aabb.mins.y, tmin, tmax))
		return false;
	if (!testSLABPlane(p1.z, d.z, aabb.mins.z, aabb.mins.z, tmin, tmax))
		return false;
	return tmin >= 0 && tmin <= glm::distance(p1, p2);
}


bool interectRayOBB(glm::vec3 p1, glm::vec3 p2, glm::vec3 d, AbstractModel::OBB obb) {
	glm::quat qinv = glm::inverse(obb.u); // Orientacion de la caja
	...
	aabb.mins = obb.c - obb.e;
	aabb.maxs = obb.c + obb.e;
	return interectRayAABB(p1, p2, d, aabb);
}
