...
float velocidad = .4f;
std::map<int, GuiTexture*> texturaVidas;
bool Jump, Juger, Speed;
...
// Texturas Gui
// - Menu
GuiTexture** textureMenu;
GuiTexture* textureMenuPlay;
GuiTexture* textureMenuExit;
...
GuiTexture* textureSpeed;
GuiTexture* textureJuger;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderGui.initialize("../Shaders/shader_gui.vs", "../Shaders/shader_gui.fs");
	...
	// - Menu
	textureMenuPlay = new GuiTexture("../Textures/ImagenesInterfaz/Inicio1.png", glm::vec2(0.f), glm::vec2(1.f));
	textureMenuExit = new GuiTexture("../Textures/ImagenesInterfaz/Inicio2.png", glm::vec2(0.f), glm::vec2(1.f));
	textureMenu = &textureMenuPlay;
	// - Exit
	...
	texturaVidas.insert({ 40, new GuiTexture("../Textures/ImagenesInterfaz/Vida02.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 60, new GuiTexture("../Textures/ImagenesInterfaz/Vida03.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 80, new GuiTexture("../Textures/ImagenesInterfaz/Vida04.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 100, new GuiTexture("../Textures/ImagenesInterfaz/Vida05.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 120, new GuiTexture("../Textures/ImagenesInterfaz/Vida06.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 140, new GuiTexture("../Textures/ImagenesInterfaz/Vida07.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 160, new GuiTexture("../Textures/ImagenesInterfaz/Vida08.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 180, new GuiTexture("../Textures/ImagenesInterfaz/Vida09.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	texturaVidas.insert({ 200, new GuiTexture("../Textures/ImagenesInterfaz/Vida10.png", glm::vec2(-0.6f, -0.8f), glm::vec2(0.35, 0.1)) });
	// - Monenita
	textureMonedita = new GuiTexture("../Textures/ImagenesInterfaz/moneda.png", glm::vec2(0.73f, 0.85f), glm::vec2(0.07, 0.07));
	textureJump = new GuiTexture("../Textures/ImagenesInterfaz/Jump.png", glm::vec2(-0.9f, 0.85f), glm::vec2(0.06, 0.06));
	textureSpeed = new GuiTexture("../Textures/ImagenesInterfaz/Speed.png", glm::vec2(-0.78f, 0.85f), glm::vec2(0.06, 0.06));
	textureJuger = new GuiTexture("../Textures/ImagenesInterfaz/Juger.png", glm::vec2(-0.66f, 0.85f), glm::vec2(0.06, 0.06));
...
bool processInput(bool continueApplication) {
	...
	if (status == MENU) {
		if (tabSelect&& glfwGetKey(window, GLFW_KEY_TAB) == GLFW_PRESS) {
			if (textureMenu == &textureMenuPlay)
				textureMenu = &textureMenuExit;
			else
				textureMenu = &textureMenuPlay;
			tabSelect = false;
		}
		...
		if (spaceSelect && glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) {
			if (textureMenu == &textureMenuPlay)
				status = GAME;
			else
				return false;
		...
		if (tabSelect && glfwGetKey(window, GLFW_KEY_TAB) == GLFW_PRESS) {
			if (textureExit == &textureExitMenu)
				textureExit = &textureExitExit;
			else
				textureExit = &textureExitMenu;
			tabSelect = false;
		}
...
void renderGUI() {
	...
	if (status == MENU) {
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, (*textureMenu)->texture->id);
		shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr((*textureMenu)->matrix));
		...
		// - Vidas
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texturaVidas.find(vidaActual)->second->texture->id);
		shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr(texturaVidas.find(vidaActual)->second->matrix));
		glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		// - Moneda
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureMonedita->texture->id);
		shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr(textureMonedita->matrix));
		glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		// - Jump
		if (Jump) {
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, textureJump->texture->id);
			shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr(textureJump->matrix));
			glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		}
		// - Speed
		if (Speed) {
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, textureSpeed->texture->id);
			shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr(textureSpeed->matrix));
			glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		}
		// - Juger
		if (Juger) {
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, textureJuger->texture->id);
			shaderGui.setMatrix4("transformationMatrix", 1, false, glm::value_ptr(textureJuger->matrix));
			glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		}
	}
	glEnable(GL_DEPTH_TEST);
	...
	if (status == GAME) {
		glEnable(GL_BLEND);
		modelText->render(std::to_string(puntos), 0.8, 0.8, 45, 0.976470f, 0.658823f, 0.098039f);
		glDisable(GL_BLEND);
	}
}
...
void resetGame() {
	puntos = 0;
	vidaMaxima = vidaActual = 100;
	Jump = Juger = Speed = false;
	moneda->InitMatrices(&terrain);
	modelMatrixSpider = glm::mat4(1.f);
	timeDay = 0.f;
}
